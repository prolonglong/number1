...
# print("share_key", len(share_key))

def test():
    plaintext = 11000
    plaintext2 = 110
    c_text = encipher(plaintext, pk)  ###加密
    c_text2 = encipher(plaintext2, pk)  ###加密

    c_text3 = addition(c_text, c_text2, pk)
    ...
    #####部分解密
    start = time.time()
    for user in range(thre):
        pa_c.append(partial_decryption(share_key[user], c_text3, pk[0]))
    end = time.time()
    t = (end - start) / user_num
    print(t)
    ...
    start = time.time()
    p1_2 = decipher(c_text3, pk, sk)  ####单独解密，用到了私钥sk
    end = time.time()
    print(end - start)
